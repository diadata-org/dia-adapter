...
import {base_decode} from "../near-api/utils/serialize.js"
import {KeyPairEd25519} from "../near-api/utils/key-pair.js"

import * as sha256 from "../near-api/utils/sha256.js"
...
//util.inspect - nodejs console object dump
...
async function main() {
  ...
  try {
    ...
    console.log(`${network.currentInfo().explorerUrl}transactions/${result.transaction.hash}`);
    console.log('--------------------------------------------------------------------------------------------');
  } catch (error) {
    console.log(error);
  };
